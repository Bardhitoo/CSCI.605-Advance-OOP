...
abstract class ChessPiece {
     ...
     */
    ChessPiece(int size, int dimensions, char[][][] theBoard) {
        MAX_SIZE = size;
        DIMENSIONS = dimensions;
        ChessPiece.theBoard = theBoard;
    }
    ...
    public int countPiece(char chessPiece) {
        int cntr = 0;
        for (int h = 0; h < (DIMENSIONS == 3 ? MAX_SIZE : 1); h++) {
...
class Rook extends ChessPiece {
     ...
     */
    Rook(int size, int dimensions, char[][][] theBoard) {
    ...
    boolean isAbleToMove(int rowPieceLoc, int colPieceLoc, int heightPieceLoc, char chessPiece) {
        ...
        for (int height = 0; height < (DIMENSIONS == 3 ? MAX_SIZE : 1); height++) {
            // Check if the piece is getting scared of itself
...
class Bishop extends ChessPiece {
    ...
    boolean isAbleToMove(int rowPieceLoc, int colPieceLoc, int heightPieceLoc, char chessPiece) {
        ...
        for (int coor = coords[coorIdOfSmaller]; coords[0] + coor < MAX_SIZE && coords[1] + coor < MAX_SIZE; coor++) {
        ...
        for (int height = 1; height < (DIMENSIONS == 3 ? MAX_SIZE : 1); height++) {
            for (int row = rowPieceLoc - height; row <= rowPieceLoc + height; row = row + height) {
                for (int column = colPieceLoc - height; column <= colPieceLoc + height; column = column + height) {
                    if (row < MAX_SIZE && row >= 0) {
                        if ((heightPieceLoc + height < MAX_SIZE) & (column < MAX_SIZE && column >= 0)
                                && (theBoard[row][column][heightPieceLoc + height] == chessPiece)) {
                            return false;
                        }
                        if ((heightPieceLoc - height >= 0) && (column < MAX_SIZE && column >= 0)
                                && (theBoard[row][column][heightPieceLoc - height] == chessPiece)) {
                            return false;
                        }
                    }
